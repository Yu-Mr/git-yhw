...
{  
	u8 temp,t,t1;
    u8 y0=y;
    u8 csize=(size/8+((size%8)?1:0))*(size/2)*2;      //得到字体一个字符对应点阵集所占的字节数
    //chr=chr-' ';//得到偏移后的值        
    for(t=0;t<csize;t++)
    {   
		
        if(size==12)temp=chr[t];         //调用1206字体
        else if(size==16)temp=chr[t];     //调用1608字体
        else if(size==24)temp=chr[t];     //调用2412字体
        else return;                                //没有的字库
        for(t1=0;t1<8;t1++)
        {
